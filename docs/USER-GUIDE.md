...
The Schema dashboard inventories JSON-LD, Microdata, and RDFa; distinguishes JSON syntax from local structural/completeness checks; and reports malformed structural values and supported content-property gaps. SCOPE does not declare unfamiliar `@type` strings invalid or require optional properties such as `Organization.url`: Schema.org permits extensions and its official validator is authoritative for vocabulary validity. When malformed JSON-LD cannot be parsed, SCOPE attempts to recover its declared `@type` from the raw block and labels it as a **likely type**, never as validated; if recovery is impossible, the dashboard says so explicitly. “JSON-LD parsed” means only that JSON syntax parsed successfully—not that the graph is fully valid. Every reported page includes dynamic links to **Validate schema** in the official Schema.org Markup Validator and **Test Google rich results** in Google’s official Rich Results Test, with the page URL prefilled. SCOPE’s local checks and suggestions are useful screening evidence, not proof of complete validity or Google rich-result eligibility. Use Schema.org validation for the current vocabulary and Google’s Rich Results Test or URL Inspection evidence for Google-specific eligibility.
...
The Trends dashboard defaults to **Like-for-like audits**, matching the latest audit's scan type and analyzed-page count. Choose an audit-date window (all retained dates, 30 days, 90 days, six months, or one year) to focus the comparison. **All runs (exploratory)** is available when you intentionally want to inspect mixed crawl populations, but it is not a reliable basis for declaring improvement or decline.

Issue severity and finding movement are normalized per 100 analyzed pages so a 50-page quick scan is not visually treated like a 1,300-page full crawl. Each chart line uses its own scale to make direction legible; compare the values in the legend and audit table rather than the apparent vertical distance between different metrics. Audit dates appear directly on charts. The run table discloses each crawl's scan type, page population, and exact GSC and GA4 periods. SCOPE charts GSC position across runs only when their reporting windows have the same duration; incompatible or unavailable periods appear as gaps rather than invented zeros.

Only compare runs with compatible scope and modules. SCOPE labels partial or non-comparable runs and keeps provider trend series separate from crawl and Google first-party series. When only one like-for-like audit exists, the executive summary explicitly asks for another comparable audit instead of inferring a direction from incompatible evidence.
