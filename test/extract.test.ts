...
test('does not overrule the official validator for extension types and inventories embedded structured data', () => {
...
test('does not require an Organization URL for Schema.org validity', () => {
  const html = '<title>Schema</title><h1>Schema</h1><script type="application/ld+json">{"@type":"BlogPosting","headline":"Test","datePublished":"2026-08-27","author":{"@type":"Person","name":"Jamie"},"publisher":{"@type":"Organization","name":"SCOPE"}}</script>';
  const result = extractPage('https://example.test/post/', 'https://example.test/post/', 200, 'text/html', html, new Headers());
  assert.ok(!result.findings.some(finding => /Organization is missing core property.*url/i.test(finding.message)));
});
