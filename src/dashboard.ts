...
const layoutLaunch=()=>document.querySelector('.launch').style.gridTemplateColumns=window.innerWidth<800?'1fr':'minmax(260px,1.4fr) minmax(220px,1fr) auto auto auto';layoutLaunch();addEventListener('resize',layoutLaunch);
...
const heads={pages:['URL','HTTP','SEO title','Description chars','Words','Internal / external','Findings'],technical:['Type','Source page','Anchor text','Destination','HTTP','Details'],aio:['Page','Readiness score','Label','Extractability /20','Evidence /20','Entity clarity /15','Opportunities'],keywords:['Keyword','Target score','Confidence','Primary page','Organic rank'],images:['Page','Current filename','Issue','Suggested filename','Suggested alt text','Basis'],findings:['Severity','Area','Finding','Page']};
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/audits') {
      ...
      const config: AuditConfig = { startUrl, excludePaths: Array.isArray(input.excludePaths) ? input.excludePaths.map(String) : [], maxPages: input.maxPages === null ? null : Number(input.maxPages ?? 50), maxKeywords: 100, concurrency: 1, delayMs: 250, userAgent: 'SCOPEOrganicAuditor/0.2 (+respectful SEO audit)', pageSpeed: Boolean(input.pageSpeed), pageSpeedApiKey: process.env.PAGESPEED_API_KEY,
