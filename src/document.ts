...
export async function createAuditDocument(report: AuditReport): Promise<Buffer> {
  ...
  const children: Array<Paragraph | Table> = [
    ...
    body(`Imported data: ${report.importedData.gscRows} GSC row(s) and ${report.importedData.ga4Rows} GA4 landing-page row(s).${report.importedData.gscAveragePosition !== undefined ? ` Impression-weighted average GSC position: ${report.importedData.gscAveragePosition.toFixed(1)}.` : ''}${report.importedData.gscProperty ? ` Search Console property: ${report.importedData.gscProperty}.` : ''} GSC reporting period: ${periodText(report.importedData.gscDateRange, 'unavailable')}. GA4 reporting period: ${periodText(report.importedData.ga4DateRange, 'unavailable')}.${report.importedData.ga4Property ? ` GA4 property: ${report.importedData.ga4Property}.` : ''}${report.importedData.ga4DataQuality?.timeZone ? ` GA4 property time zone: ${report.importedData.ga4DataQuality.timeZone}.` : ''}${report.importedData.ga4DataQuality?.subjectToThresholding ? ' Google reported privacy thresholding for the GA4 response.' : ''}${report.importedData.ga4DataQuality?.dataLossFromOtherRow ? ' Google reported high-cardinality data loss from an “other” row.' : ''}${report.importedData.ga4DataQuality?.sampled ? ' Google reported sampling.' : ''} OAuth credentials and tokens are stored locally only and are never retained in report configuration or output files.`),
  ...
  const ga4Totals = report.importedData.ga4Totals;
  children.push(body(`Reporting period: ${periodText(report.importedData.ga4DateRange, 'unavailable - connect the GA4 Data API or set the CSV dates in Connected data')}.${report.importedData.ga4Property ? ` Property: ${report.importedData.ga4Property}.` : ''} Rows are sorted by sessions from greatest to least. ${ga4Totals ? `Property totals returned by the GA4 Data API: ${ga4Totals.sessions} sessions, ${ga4Totals.totalUsers} distinct Total users, ${ga4Totals.engagementRate == null ? 'engagement rate unavailable' : `${(ga4Totals.engagementRate * 100).toFixed(1)}% engagement rate`}, and ${ga4Totals.bounceRate == null ? 'bounce rate unavailable' : `${(ga4Totals.bounceRate * 100).toFixed(1)}% bounce rate`}.` : 'CSV fallback metrics below reflect matched landing-page rows; summing Total users across rows can count a person more than once.'} Omitted metrics are labeled unavailable rather than zero.`));
  if (!ga4Pages.length) children.push(body('No GA4 landing-page rows matched analyzed pages.'));
  else children.push(dataTable(
    ['Landing page', 'Sessions', 'Total users', 'Engaged sessions', 'Engagement rate', 'Bounce rate', 'Key events'],
