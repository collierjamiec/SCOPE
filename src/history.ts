...
export async function getLatestDomainAuditConfig(domainId: string) {
  ...
  const allowed = ['maxPages','maxKeywords','maxRankings','concurrency','delayMs','pageSpeed','pageSpeedConcurrency','pageSpeedSkipArchives','pageSpeedExcludePaths','excludePaths','maxDepth','maxUrlsPerPath','stripTrackingParameters','renderJavaScript','sitemapOnly','excludeArchives','externalCrawlDepth','maxExternalPages','analyzeImages','reportBrokenLinks','analyzeSchema'];
...
const trafficTotals = (report: AuditReport) => {
  ...
  const totals = report.importedData.ga4Totals;
  return { gscClicks: observed.length ? gscClicks : null, gscImpressions: observed.length ? gscImpressions : null, gscCtr: gscImpressions ? gscClicks / gscImpressions : null, gscKeywordCount: observed.length || null, ga4Sessions: totals?.sessions ?? (analytics.length ? ga4Sessions : null), ga4Users: totals?.totalUsers ?? (analytics.length ? ga4Users : null), ga4EngagementRate: totals?.engagementRate ?? (ga4Sessions ? engaged / ga4Sessions : null), ga4BounceRate: totals?.bounceRate ?? (bounceSessions ? bounceRows.reduce((sum, item) => sum + item.bounceRate! * item.sessions, 0) / bounceSessions : null) };
};
