...
export function applySitewideStructuralMetrics(pages: PageResult[], excluded: Array<{ url: string; reason: string; status?: number }>) {
  ...
  pages.forEach((page, index) => {
    for (const value of [page.title, page.metaDescription].filter(Boolean)) { const key = `${page.pageType}:${value.toLowerCase()}`, matches = metadata.get(key) ?? []; matches.push(index); metadata.set(key, matches); }
  ...
  const directMatches = new Map<number, Set<number>>(), pair = (left: number, right: number) => { if (left === right) return; (directMatches.get(left) ?? directMatches.set(left, new Set()).get(left)!).add(right); (directMatches.get(right) ?? directMatches.set(right, new Set()).get(right)!).add(left); };
  for (const indexes of metadata.values()) if (indexes.length > 1) for (let left = 0; left < indexes.length; left++) for (let right = left + 1; right < indexes.length; right++) pair(indexes[left], indexes[right]);
  for (const indexes of buckets.values()) for (let left = 0; left < indexes.length; left++) for (let right = left + 1; right < indexes.length; right++) { const a = pages[indexes[left]], b = pages[indexes[right]]; if (a.contentFingerprint && b.contentFingerprint && distance(a.contentFingerprint, b.contentFingerprint) <= 4) pair(indexes[left], indexes[right]); }
  for (const [index, matches] of directMatches) { const archive = String(pages[index].pageType).includes('archive'), urls = [...matches].map(item => pages[item].url); pages[index].findings.push({ category: 'seo', severity: archive ? 'info' : 'warning', rule: archive ? 'archive_template_similarity' : 'near_duplicate_content', message: archive ? `This archive shares expected listing-template content with ${urls.length} other archive page${urls.length === 1 ? '' : 's'}; review only if the listings serve the same search intent.` : `This page has direct similarity evidence against ${urls.length} specific analyzed page${urls.length === 1 ? '' : 's'}.`, evidence: urls.join(' | ') }); }
  return { canonicalChains, canonicalNon200Targets, nearDuplicateGroups: Math.round([...directMatches.values()].reduce((sum, matches) => sum + matches.size, 0) / 2) };
}
  ...
  if (input.pageSpeed && !control.isCancelled()) {
    ...
    try {
      const worker = async (workerIndex: number) => {
        if (workerIndex > 0) await Promise.race([sleep(workerIndex * 125), new Promise<void>(resolve => pageSpeedAbort.signal.addEventListener('abort', () => resolve(), { once: true }))]);
        while (!control.isCancelled()) {
          const index = nextPage++;
          if (index >= eligiblePages.length) return;
          const page = eligiblePages[index];
          await onProgress({ phase: 'pagespeed', message: `Running PageSpeed for ${page.url}`, fetched: fetched.size, analyzed: pages.length, queued: Math.max(0, eligiblePages.length - completed), stageCompleted: completed, stageTotal: eligiblePages.length, currentUrl: page.url, percent: 92 + Math.round((completed / Math.max(1, eligiblePages.length)) * 5), ...findingCounts() });
          if (pageSpeedRateLimited) {
            page.pageSpeed = [{ strategy: 'mobile', performance: null, accessibility: null, bestPractices: null, seo: null, metrics: {}, fieldMetrics: {}, errorCode: 'skipped_after_rate_limit', error: 'Skipped because Google PageSpeed quota was exhausted earlier in this audit. Retry later or configure a PageSpeed API key with available quota.' }];
          } else {
          ...
          completed += 1;
          await onProgress({ phase: 'pagespeed', message: `Completed PageSpeed for ${page.url}`, fetched: fetched.size, analyzed: pages.length, queued: Math.max(0, eligiblePages.length - completed), stageCompleted: completed, stageTotal: eligiblePages.length, currentUrl: page.url, percent: 92 + Math.round((completed / Math.max(1, eligiblePages.length)) * 5), ...findingCounts() });
        }
      };
      await Promise.all(Array.from({ length: Math.min(pageSpeedConcurrency, Math.max(1, eligiblePages.length)) }, (_, index) => worker(index)));
    } finally { clearInterval(cancellationWatcher); }
    completed = eligiblePages.filter(page => page.pageSpeed.length > 0).length;
