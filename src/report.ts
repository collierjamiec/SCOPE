...
export function pagesCsv(report: AuditReport): string {
  const header = ['URL','Status','Response Time Ms','Redirect Chain','Internal Links','External Links','Incoming Internal Links','Images','Images Missing Alt Text','AI Readiness Score','AI Readiness Label','AI Extractability','AI Evidence','AI Entity Clarity','AI Intent Coverage','AI Freshness','AI Multimodal','HTML Lang','Viewport Meta','Canonical','Canonical Matches URL','SEO Title','Title Characters','Meta Description','Meta Description Characters','H1','H2s','Primary CTA','CTA URL','Detected Schema Types','Suggested Schema Types','Word Count','Findings'];
  const rows = report.pages.map(page => [
    ...
    page.incomingInternalLinks, page.imageCount, page.imagesMissingAltText,
...
export function keywordsCsv(report: AuditReport): string {
  ...
  const rows = report.keywords.map(keyword => [
    keyword.keyword, keyword.pages[0]?.score ?? '', keyword.confidence, keyword.pages[0]?.url ?? '', keyword.pages.slice(1).map(p => p.url).join(' | '),
