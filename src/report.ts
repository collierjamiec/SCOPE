...
export function pagesCsv(report: AuditReport): string {
  ...
  const rows = report.pages.map(page => [
    ...
    page.incomingInternalLinks, page.clickDepth ?? '', page.orphan ?? false, page.analytics?.sessions ?? '', page.analytics?.activeUsers ?? '', page.analytics?.engagedSessions ?? '', page.analytics?.engagementRate ?? '', page.analytics?.keyEvents ?? '', page.imageCount, page.imagesMissingAltText,
