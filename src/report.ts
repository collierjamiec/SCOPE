...
export function pagesCsv(report: AuditReport): string {
  const header = ['URL','Page Type','Status','Response Time Ms','Redirect Chain','Internal Links','External Links','Incoming Internal Links','Click Depth','Orphan Page','GA4 Sessions','GA4 Total Users','GA4 Engaged Sessions','GA4 Engagement Rate','GA4 Bounce Rate','GA4 Key Events','Images','Images Missing Alt Text','AIO Readiness Score','AIO Readiness Label','AIO Extractability','AIO Evidence','AIO Entity Clarity','AIO Intent Coverage','AIO Freshness','AIO Multimodal','Direct Answer Pairs','Definition Passages','Comparison Structures','Attributed Quotes','Unattributed Quotes','Numeric Claims','Original Data Claims','Vague Claims','Recognizable Primary Sources','Volatile Claims','Data-rich Images','Data-rich Images With Context','HTML Lang','Viewport Meta','Canonical','Canonical Matches URL','SEO Title','Title Characters','Meta Description','Meta Description Characters','H1','H2s','Primary CTA','CTA URL','Detected Schema Types','Suggested Schema Types','Declared Entities','Word Count','Sentence Count','Paragraph Count','Average Words Per Sentence','Flesch Reading Ease','Flesch-Kincaid Grade','Reading Time Minutes','Text to HTML Ratio','Findings'];
  const rows = report.pages.map(page => [
    ...
    page.aio?.advancedSignals?.directAnswerPairs ?? '', page.aio?.advancedSignals?.definitionPassages ?? '', page.aio?.advancedSignals?.comparisonStructures ?? '', page.aio?.advancedSignals?.attributedQuotes ?? '', page.aio?.advancedSignals?.unattributedQuotes ?? '', page.aio?.advancedSignals?.numericClaims ?? '', page.aio?.advancedSignals?.originalDataClaims ?? '', page.aio?.advancedSignals?.vagueClaims ?? '', page.aio?.advancedSignals?.recognizablePrimarySources ?? '', page.aio?.advancedSignals?.volatileClaims ?? '', page.aio?.advancedSignals?.dataRichImages ?? '', page.aio?.advancedSignals?.dataRichImagesWithContext ?? '',
    ...
    page.schemas.flatMap(schema => schema.types).join(' | '), page.suggestedSchemas.map(schema => schema.type).join(' | '), (page.entityNames ?? []).map(entity => `${entity.type}: ${entity.name}`).join(' | '), page.wordCount, page.contentMetrics.sentenceCount, page.contentMetrics.paragraphCount, page.contentMetrics.averageWordsPerSentence, page.contentMetrics.fleschReadingEase ?? '', page.contentMetrics.fleschKincaidGrade ?? '', page.contentMetrics.readingTimeMinutes, page.contentMetrics.textToHtmlRatio,
