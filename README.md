...
The Schema dashboard inventories JSON-LD, Microdata, and RDFa and includes a per-page link to the official Schema.org Markup Validator. SCOPE distinguishes parseable JSON from semantic validity, checks nested `@type` values and supported core-property gaps, and identifies malformed blocks by a best-effort, explicitly unvalidated likely `@type` when recoverable. It explicitly separates local screening from independent Schema.org validation and Google-specific rich-result eligibility.
